...
Fully compatible with util-linux `rev` for ASCII input. Output is byte-identical to util-linux `rev` for single lines, multiple lines, empty input, and single/multiple file operands (verified in the `yupsh/bin-rev` Docker integration harness against Debian's util-linux `rev`).
